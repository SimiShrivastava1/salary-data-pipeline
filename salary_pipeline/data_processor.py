...
class EnhancedSmartFileProcessor:
    """Data processing with DuckDB integration and improved Glassdoor location/seniority mapping"""
    ...
    def load_all_data_sources(self):
        ...
        if self.config.enable_indeed:
            indeed_data = self._load_source_data('indeed', self.config.indeed_path)
        ...
        if self.config.enable_simplyhired:
            sh_data = self._load_source_data('simplyhired', self.config.simplyhired_path)
        ...
        if self.config.enable_linkedin:
            linkedin_data = self._load_source_data('linkedin', self.config.linkedin_path)
    ...
    def _load_source_data(self, source_name: str, folder_path: str):
        """Load data from parquet source"""
        files = self.find_files_in_date_range(folder_path)
        if not files:
            return None

        chunk_size = self.memory_mgr.calculate_optimal_chunk_size(len(files))
        all_chunks = []
    ...
    def _process_source_dataframe(self, df: pd.DataFrame, source_name: str):
        ...
        try:
            if source_name in ['indeed', 'simplyhired', 'linkedin']:
                ...
                # Indeed has good location data, SimplyHired/LinkedIn may have empty location data
